...
class ViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
  ...
  override func viewDidLoad() {
    ...
    let js = "window.ios={sharefunc:function(){window.webkit.messageHandlers.sharefunc.postMessage({body: '6436326363246'});}};"
    
//    let sharescript = WKUserScript(source: "function sharefunc() { window.webkit.messageHandlers.sharefunc.postMessage({body: 'call js alert in js'});var response = prompt();document.getElementById('jsParamFuncSpan').innerHTML = response; window.open('http://www.baidu.com')}",
...
//        forMainFrameOnly: true) // 只添加到mainFrame中
    ...
    configuretion.userContentController.addUserScript(sharescript)
        // 添加一个名称，就可以在JS通过这个名称发送消息：
    // window.webkit.messageHandlers.AppModel.postMessage({body: 'xxx'})
    configuretion.userContentController.add(self, name: "sharefunc")
...
//  self.present(alert, animated: true, completion: nil)
    ...
    func getUserinfo(completionHandler: @escaping (String?) -> Void)  {
    
        
        completionHandler("这是测试方法")
